...
// ---------------------------------------------------------------------------
// Proposal (reviewable artifact — does NOT mutate the tree)
// ---------------------------------------------------------------------------
...
/**
 * Build a deterministic, reviewable Markdown proposal for the next engineering
 * improvement: current autonomy/state, discovered gaps, prioritized tasks (top
 * first), the next validated action (FINAL PRINCIPLE) and the recommended
 * loop-piv style plan. Safe to persist (`.ultraia/genesis/proposal.md`); it never
 * edits source or commits.
 */
export function buildGenesisProposal(input: GenesisProposalInput): GenesisProposal {
  const { manifest, state, tasks = [], gaps = [] } = input;
  const next = nextEngineeringAction(manifest, state, tasks);
  const ranked = prioritizeTasks(tasks);
  const plan = buildGenesisPlan(manifest, state, tasks);
  const proj = manifest.project;
  const auto = autonomyLevel(manifest);

  const lines: string[] = [];
  lines.push(`# Genesis Proposal — ${proj?.name ?? manifest.project?.id ?? 'project'}`);
  lines.push('');
  lines.push(`> Generated by the Genesis engine. Review before acting — this artifact does`);
  lines.push(`> not modify the repository.`);
  lines.push('');
  lines.push(`- **Project:** ${proj?.id ?? 'n/a'}${proj?.name ? ` (${proj.name})` : ''}`);
  lines.push(`- **Objective:** ${manifest.objective?.primary ?? '(none declared)'}`);
  lines.push(`- **Autonomy level:** ${auto} / 3`);
  lines.push(`- **Iteration:** ${state.iterations ?? 0}`);
  lines.push(`- **Gates required:** ${qualityGates(manifest).filter((g) => g.required).map((g) => g.name).join(', ') || '(none)'}`);
  lines.push('');
  lines.push(`## Discovered gaps (${gaps.length})`);
  if (gaps.length === 0) {
    lines.push('_No gaps discovered._');
  } else {
    for (const g of gaps.slice(0, 12)) {
      lines.push(`- \`${g.kind}\`: ${g.detail ?? g.id}`);
    }
    if (gaps.length > 12) lines.push(`- _…and ${gaps.length - 12} more._`);
  }
  lines.push('');
  lines.push(`## Prioritized tasks (${ranked.length})`);
  if (ranked.length === 0) {
    lines.push('_No tasks queued — following pipeline steps._');
  } else {
    for (const t of ranked.slice(0, 12)) {
      lines.push(
        `- \`${t.id}\` (priority ${t.priority.toFixed(4)}${t.bloqueador ? ', BLOCKER' : ''}): ${t.descripcion}`,
      );
    }
    if (ranked.length > 12) lines.push(`- _…and ${ranked.length - 12} more._`);
  }
  lines.push('');
  lines.push(`## Next validated action (FINAL PRINCIPLE)`);
  lines.push(`- **Action:** \`${next.action}\``);
  lines.push(`- **Rationale:** ${next.rationale}`);
  lines.push('');
  lines.push(`## Recommended plan`);
  for (const step of plan.pasos) lines.push(`- ${step}`);
  lines.push('');
  lines.push(`_Scoped gate: ${plan.criterios_scoped} · Full gate: ${plan.criterios_full}_`);

  return {
    markdown: lines.join('\n'),
    nextAction: next.action,
    topTaskId: ranked[0]?.id,
  };
}
