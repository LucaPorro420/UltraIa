# librosgratis.dev — 115 libros gratuitos de programación (midudev/libros-programacion-gratis)

Fuente: https://github.com/midudev/libros-programacion-gratis (librosgratis.dev), enlaces.txt L826.
Descargado vía curl el 18/08/2026 (20.225 bytes, UTF-8 limpio).
Mapeo implementado en packages/core/src/tools/libros.ts (capability libros, 31 tests).
NOTA: el resumen por categorías del README suma 114 (Fundamentos '13') mientras el índice
de secciones suma 115 — la implementación computa los conteos de los datos.

---

# librosgratis.dev

Biblioteca viva de libros y guías gratuitas de programación en español. El catálogo de la web se mantiene en `web/src/data/library.ts` y este README refleja la misma lista para que sea fácil revisar, buscar y proponer cambios desde GitHub.

- Web: [librosgratis.dev](https://librosgratis.dev)
- Repositorio fuente original: [midudev/libros-programacion-gratis](https://github.com/midudev/libros-programacion-gratis)
- Total actual: 115 recursos en 32 secciones

## Categorías

- Fundamentos: 13 recursos en 4 secciones
- Desarrollo web: 4 recursos en 1 sección
- Lenguajes: 71 recursos en 15 secciones
- Plataformas: 2 recursos en 1 sección
- Frameworks: 9 recursos en 5 secciones
- Herramientas: 8 recursos en 3 secciones
- Bases de datos: 6 recursos en 2 secciones
- IA y datos: 1 recurso en 1 sección

## Índice

- [Generales](#generales) · 3
- [Algoritmos y estructuras de datos](#algoritmos) · 8
- [HTML y CSS](#html-css) · 4
- [JavaScript](#javascript) · 13
- [TypeScript](#typescript) · 7
- [Python](#python) · 13
- [Ruby](#ruby) · 3
- [Rust](#rust) · 5
- [Blockchain](#blockchain) · 4
- [PHP](#php) · 4
- [Haskell](#haskell) · 3
- [Golang](#golang) · 2
- [Kotlin](#kotlin) · 2
- [Android](#android) · 2
- [C](#c) · 1
- [C++](#cplusplus) · 4
- [C#](#csharp) · 2
- [Java](#java) · 6
- [R](#r) · 2
- [React](#react) · 4
- [Qwik](#qwik) · 1
- [Node.js](#nodejs) · 1
- [Angular](#angular) · 1
- [Django](#django) · 2
- [Git](#git) · 4
- [Docker](#docker) · 2
- [Linux y terminal](#linux) · 2
- [SQL](#sql) · 4
- [NoSQL](#nosql) · 2
- [Sistemas operativos](#sistemas-operativos) · 1
- [Inteligencia Artificial](#ia) · 1
- [Metodologías de desarrollo](#metodologias) · 2

## Generales

<a id="generales"></a>

Para abrir apetito, mejorar criterio y aprender a pensar mejor como programador.

- [97 cosas que todo programador debe saber](https://97cosas.com/programador/) — Kevlin Henney
- [100cosasdev](https://100cosas.dev/) — midudev
- [Los apuntes de Majo](https://losapuntesdemajo.vercel.app/) — Majo Ledesma

## Algoritmos y estructuras de datos

<a id="algoritmos"></a>

Lógica, pseudocódigo, POO, análisis de algoritmos y estructuras para resolver mejor.

- [Diseño de Algoritmos en Pseudocódigo y Ordinogramas](https://librosgratis.dev/books/algoritmos-pseudocodigo-ordinogramas.pdf) — Carlos Pes · PDF
- [Estructuras de datos](https://librosgratis.dev/books/estructuras-de-datos.pdf) — Luis Fernando Zapata Alvarez · PDF
- [Problemas y Algoritmos](https://librosgratis.dev/books/problemas-y-algoritmos.pdf) — Luis E. Vargas Azcona · PDF
- [Las bases conceptuales de la Programación](https://librosgratis.dev/books/bases-conceptuales-programacion.pdf) — Pablo E. “Fidel” Martínez López · PDF
- [Introducción a la Lógica de Programación](https://librosgratis.dev/books/logica-de-programacion.pdf) — Jorge O. Herrera M., Julián E. Gutiérrez P., Robinson Pulgarín G. · PDF
- [Fundamentos de la programación](https://librosgratis.dev/books/fundamentos-programacion.pdf) — Luis Hernández Yáñez · PDF
- [Introducción a la programación orientada a objetos](https://librosgratis.dev/books/introduccion-poo.pdf) — Vicent Moncho Mas · PDF
- [Apuntes de Estructuras de Datos y Algoritmos](https://librosgratis.dev/books/apuntes-estructuras-datos-algoritmos.pdf) — Javier Campos · PDF

## HTML y CSS

<a id="html-css"></a>

Maquetación, estilos e interfaces web desde fundamentos hasta CSS moderno.

- [Diseño de Interfaces Web](http://interfacesweb.github.io/unidades/) — Pedro Prieto · HTML
- [Estructura con CSS](https://es.learnlayout.com/) — Learn CSS Layout, traducido al español · HTML
- [MDN: HTML](https://developer.mozilla.org/es/docs/Web/HTML) — MDN Web Docs · HTML
- [MDN: CSS](https://developer.mozilla.org/es/docs/Web/CSS) — MDN Web Docs · HTML

## JavaScript

<a id="javascript"></a>

La puerta de entrada a la web moderna, desde fundamentos hasta buenas prácticas.

- [JavaScript elocuente (Cuarta edición)](https://librosgratis.dev/books/javascript-elocuente-cuarta-edicion.pdf) — Marijn Haverbeke · PDF
- [JavaScript, ¡Inspírate!](https://leanpub.com/javascript-inspirate) — Ulises Gascón · eBook
- [JavaScript Moderno](https://es.javascript.info/) — Ilya Kantor · HTML
- [You Don’t Know JS (traducción al español)](https://github.com/You-Dont-Know-JS-ES/Traduccion) — Kyle Simpson, traducido por You-Dont-Know-JS-ES · HTML
- [MDN: Guía de JavaScript](https://developer.mozilla.org/es/docs/Web/JavaScript/Guide) · HTML
- [Learn JavaScript](https://javascript.sumankunwar.com.np/es) — Suman Kunwar · HTML
- [Introducción a JavaScript](https://librosgratis.dev/books/javascript-introduccion-eguiluz.pdf) — Javier Eguíluz Pérez · PDF
- [JavaScript](https://librosgratis.dev/books/javascript-uoc.pdf) — Jordi Collell Puig y Anna Ferry Mestres · PDF
- [Asincronismo en JavaScript](https://librosgratis.dev/books/javascript-asincronismo.pdf) — Charly Cimino · PDF
- [Fundamentos de jQuery](https://librosgratis.dev/books/jquery-fundamentos.pdf) — Rebecca Murphey, traducido por Leandro D’Onofrio · PDF
- [CSS3 y Javascript avanzado](https://librosgratis.dev/books/css3-javascript-avanzado.pdf) — Jordi Collell Puig · PDF
- [Full Stack Open](https://fullstackopen.com/es/) — Universidad de Helsinki, traducido por Sebastian Torres, Cynthia Vico Vacca y Pablo Maffioli · HTML
- [Clean Code JavaScript en Español](https://github.com/andersontr15/clean-code-javascript-es) — Ryan McDermott, traducido por Theodore Anderson · HTML

## TypeScript

<a id="typescript"></a>

Tipos, tooling y confianza para escalar aplicaciones front y back.

- [Introducción a TypeScript](https://khru.gitbooks.io/typescript/) — Emmanuel Valverde Ramos · HTML
- [TypeScript en Profundidad](https://github.com/melissarofman/typescript-book) — Basarat Ali Syed, traducido por Melissa Rofman · HTML
- [Introducción a TypeScript](https://mega.nz/file/TldlTZID#1A90Wn8xYloDvekX8rQewI3Yh8HMJXlufRUEWEcOzNU) — Adictos al trabajo
- [TypeScript para Principiantes](https://mega.nz/file/7hdwEY6b#ESsixH9wCUFhUugkRq8BEa1uZlzFXCJX6QxHdL5Yz9Q) — Envato Tuts+
- [Manual de TypeScript](https://mega.nz/#!qwcFDZ7a!ggLXIZ4c-O1Do0OEuvK0Mz8k39LvYQwdaJ2LtKKxgsE) — Emmanuel Valverde y Pedro Hernández-Mora
- [Uso avanzado de TypeScript en un ejemplo real](https://neliosoftware.com/es/blog/uso-avanzado-de-typescript/) — Nelio Software · HTML
- [Aprendizaje TypeScript](https://librosgratis.dev/books/typescript-aprendizaje.pdf) — RipTutorial · PDF

## Python

<a id="python"></a>

Automatización, análisis y backend con una curva de entrada muy amable.

- [Aprende Python](https://uneweb.edu.ve/tuto-docs/libro-python.pdf) — Sergio Delgado Quintero · PDF
- [Python para todos](https://librosgratis.dev/books/python-para-todos.pdf) — Raúl González Duque · PDF
- [Aprenda a pensar como un programador con Python](https://librosgratis.dev/books/python-pensar-programador.pdf) — Allen Downey, Jeffrey Elkner, Chris Meyers · PDF
- [Python para todos, Explorando la información con Python 3](https://librosgratis.dev/books/python-explorando-informacion.pdf) — Charles R. Severance · PDF
- [Manual básico, iniciación a Python 3](https://librosgratis.dev/books/python-manual-basico.pdf) — José Miguel Ruiz Torres · PDF
- [Python Intermedio](https://librosgratis.dev/books/python-intermedio.pdf) — Comunidad ellibrodepython.com · PDF
- [Inmersión en Python 3](https://librosgratis.dev/books/python-inmersion.pdf) — Mark Pilgrim, traducido por José Miguel González Aguilera · PDF
- [Ejercicios básicos de programación resueltos en Python](https://librosgratis.dev/books/python-ejercicios-basicos.pdf) · PDF
- [Introducción a Python para cálculo científico](https://librosgratis.dev/books/python-calculo-cientifico.pdf) — A. Garcimartín · PDF
- [Introducción a la programación con Python 3](https://librosgratis.dev/books/python-introduccion-programacion-3.pdf) — Andrés Marzal Varó, Isabel Gracia Luengo, Pedro García Sevilla · PDF
- [Apuntes Python](https://librosgratis.dev/books/python-apuntes.pdf) — Manuel Vergara · PDF
- [Inventa tus propios juegos de computadora con Python](https://librosgratis.dev/books/python-inventa-juegos.pdf) — Al Sweigart · PDF
- [El tutorial de Python](https://docs.python.org/es/3/tutorial/) — Python Software Foundation · HTML

## Ruby

<a id="ruby"></a>

Sintaxis elegante y una manera de programar muy orientada a la legibilidad.

- [Aprende a programar con Ruby](http://rubysur.org/aprende.a.programar) — RubySur · HTML
- [Ruby en veinte minutos](https://www.ruby-lang.org/es/documentation/quickstart/) — Ruby · HTML
- [Introducción a Rails](http://rubysur.org/introduccion.a.rails/) — RubySur · HTML

## Rust

<a id="rust"></a>

Rendimiento, seguridad de memoria y una comunidad técnica con mucha calidad.

- [Aprendizaje Rust](https://librosgratis.dev/books/rust-aprendizaje.pdf) — RipTutorial · PDF
- [El Lenguaje de Programación Rust - 2016](https://goyox86.github.io/elpr/README.html) — Jose Narvaez · HTML
- [El Lenguaje de Programación Rust - 2024](https://book.rustlang-es.org) — Libro oficial traducido por RustLang en Español · HTML, PDF
- [Comprehensive Rust](https://google.github.io/comprehensive-rust/es/) — Google · HTML, PDF _Está planteado como workshop, muy útil para estudiar con instructor o en grupo._
- [Rust para C#/.NET Developers](https://dotnet-book.rustlang-es.org) — Microsoft, traducido por RustLang en Español · HTML, PDF _Incluye comparaciones sencillas para entender bien el cambio de mentalidad entre lenguajes._

## Blockchain

<a id="blockchain"></a>

Descentralización, contratos inteligentes y criptografía aplicada.

- [Bitcoin: Un sistema de efectivo electrónico de usuario a usuario](https://bitcoin.org/files/bitcoin-paper/bitcoin_es.pdf) — Satoshi Nakamoto · PDF
- [El Libro de Satoshi](http://www.libroblockchain.com/satoshi/) — Phil Champagne · HTML
- [Entendiendo el Blockchain](https://www.secmca.org/wp-content/uploads/2019/12/Blockchain.pdf) — SECMCA · PDF
- [Solidity: Documentación oficial en español](https://solidity-es.readthedocs.io/) · HTML

## PHP

<a id="php"></a>

Backend pragmático con mucha historia y recursos excelentes para aprender bien.

- [PHP, la manera correcta](https://phpdevenezuela.github.io/php-the-right-way/) — Josh Lockhart, Phil Sturgeon · HTML
- [Programación en PHP a través de ejemplos](https://librosgratis.dev/books/php-programacion-ejemplos.pdf) — Manuel Palomo Duarte, Ildefonso Montero Pérez · PDF
- [POO y MVC en PHP](https://librosgratis.dev/books/php-poo-mvc.pdf) — Eugenia Bahit · PDF
- [Laboratorio de PHP y MySQL](https://openlibro.com/wp-content/uploads/2026/03/laboratorio-php-mysql.pdf) — Piero Berni Millet, Dídac Gil de la Iglesia · PDF _PDF externo: supera el límite de 25 MiB por asset de Cloudflare Workers._

## Haskell

<a id="haskell"></a>

Pensamiento funcional duro y puro para expandir cómo entiendes el código.

- [Piensa en Haskell](https://librosgratis.dev/books/haskell-piensa.pdf) — José A. Alonso Jiménez, Mª José Hidalgo Doblado · PDF
- [¡Aprende Haskell por el bien de todos!](http://aprendehaskell.es/main.html) · HTML
- [Piensa en Haskell y en Python](https://librosgratis.dev/books/haskell-python-ejercicios.pdf) — José A. Alonso Jiménez · PDF

## Golang

<a id="golang"></a>

Concurrencia, simplicidad y tooling impecable para servicios y utilidades.

- [El pequeño libro de Go](https://librosgratis.dev/books/go-pequeno-libro.pdf) — Karl Seguin, traducido por Raúl Exposito · PDF
- [Go en Español](https://nachopacheco.gitbooks.io/go-es/content/doc/) — Nacho Pacheco

## Kotlin

<a id="kotlin"></a>

Android moderno y una sintaxis muy agradable para aplicaciones robustas.

- [Curso programación Android en Kotlin](https://cursokotlin.com/curso-programacion-kotlin-android/) — AristiDevs · HTML
- [Kotlin](https://librosgratis.dev/books/kotlin-stackoverflow-docs.pdf) — Stack Overflow Documentation · PDF

## Android

<a id="android"></a>

Desarrollo de aplicaciones Android con guías prácticas en español.

- [Manual Programación Android](https://aluzardo.github.io/trabajo-fin-de-grado/Tutoriales/Manual%20Programacion%20Android.pdf) — Salvador Gómez Oliver · PDF
- [Curso sobre los aspectos básicos de Android con Compose](https://developer.android.com/courses/android-basics-compose/course?hl=es-419) — Android Developers · HTML

## C

<a id="c"></a>

Fundamentos de bajo nivel, memoria y pensamiento cercano al sistema.

- [Introducción a la Programación con C](https://librosgratis.dev/books/c-introduccion-programacion.pdf) — Andrés Marzal e Isabel Gracia · PDF

## C++

<a id="cplusplus"></a>

Orientación a objetos, eficiencia y bases para software de alto rendimiento.

- [C++ estándar](https://librosgratis.dev/books/cpp-estandar.pdf) — Miguel Hernando Gutiérrez · PDF
- [Programación orientada a objetos Ejercicios propuestos con C++](https://librosgratis.dev/books/cpp-poo-ejercicios.pdf) — Cristina Cachero, Pedro J. Ponce de León · PDF
- [Fundamentos Básicos de Programación en C++](https://librosgratis.dev/books/cpp-fundamentos-basicos.pdf) — Francisco Martínez del Río · PDF
- [Curso de C++](https://conclase.net/c/curso) — Con Clase · HTML

## C#

<a id="csharp"></a>

Programación moderna sobre .NET, desde consola hasta backend web.

- [Introducción a la programación con C#](https://librosgratis.dev/books/csharp-introduccion-programacion.pdf) — Nacho Cabanes · PDF
- [El pequeño libro de ASP.NET Core](https://librosgratis.dev/books/aspnet-core-pequeno-libro.pdf) — Nate Barbettini · PDF

## Java

<a id="java"></a>

Una base muy sólida para aprender orientación a objetos y ecosistemas empresariales.

- [Fundamentos de programación en Java](https://es.slideshare.net/slideshow/java-fundamentos/23333338) — Jorge Martínez Ladrón · PDF
- [Iniciando en Java: Programación para Todos](https://librosgratis.dev/books/java-iniciando-programacion.pdf) — Julián Camilo Tuta Diaz · PDF
- [Java Apuntes Básicos](https://librosgratis.dev/books/java-apuntes-basicos.pdf) — Jorge A. López Vargas · PDF
- [Java básico para aprendices](https://librosgratis.dev/books/java-basico-aprendices.pdf) — Manuel Jesús Abanto Morales et al. · PDF
- [Introducción a la Programación Orientada a Objetos con Java](https://librosgratis.dev/books/java-introduccion-poo.pdf) — Rafael Llobet Azpitarte, Pedro Alonso Jordá, Jaume Devesa Llinares, Emili Miedes De Elías, María Idoia Ruiz Fuertes, Francisco Torres Goterris · PDF
- [Ejercicios de Programación en Java](https://librosgratis.dev/books/java-ejercicios-programacion.pdf) — Francisco Manuel Pérez Montes · PDF

## R

<a id="r"></a>

Análisis de datos y visualización para quien quiere ir directo al insight.

- [R para Ciencia de Datos](https://es.r4ds.hadley.nz/) — Hadley Wickham y Garrett Grolemund · HTML
- [Introducción a R](https://librosgratis.dev/books/r-introduccion.pdf) — Andrés González y Silvia González · PDF

## React

<a id="react"></a>

Componentes, estado y patrones para crear interfaces ricas y mantenibles.

- [React: De aprendiz a maestro](https://librosgratis.dev/books/react-aprendiz-maestro.pdf) — Raúl Expósito · PDF
- [React](https://librosgratis.dev/books/react-stackoverflow-docs.pdf) — Stack Overflow Documentation · PDF
- [Preguntas de entrevista de React.js](https://www.reactjs.wiki/) — Miguel Ángel Durán · HTML
- [Desarrollo de Aplicaciones Web con React.js y Redux.js](https://leanpub.com/read/react-redux) — Sergio Daniel Xalambrí · HTML

## Qwik

<a id="qwik"></a>

Performance extrema y carga diferida para experiencias rapidísimas.

- [Qwik: Desde cero a producción](https://qwik-book-spanish.netlify.app/) — Anartz Mugika · HTML

## Node.js

<a id="nodejs"></a>

Backend JavaScript, asincronía y fundamentos para escribir servicios con Node.

- [Node Beginner Book](https://www.nodebeginner.org/index-es.html) — Manuel Kiessling · HTML

## Angular

<a id="angular"></a>

Arquitectura frontend con TypeScript, componentes y patrones de aplicación.

- [Entendiendo Angular](https://jorgeucano.gitbook.io/entendiendo-angular/) — Jorge Cano · HTML

## Django

<a id="django"></a>

Backend web con Python, desde el tutorial oficial hasta proyectos guiados.

- [Django documentation](https://docs.djangoproject.com/es/stable/) — Django Software Foundation · HTML
- [Tutorial de Django Girls](https://tutorial.djangogirls.org/es/) — Django Girls · HTML

## Git

<a id="git"></a>

Versionado bien aprendido para colaborar sin miedo a romper nada.

- [Pro Git](https://librosgratis.dev/books/git-pro.pdf) — Scott Chacon y Ben Straub · PDF
- [Git, la guía sencilla](https://rogerdudler.github.io/git-guide/index.es.html) — Roger Dudler · HTML
- [Git Immersion en español](https://esparta.github.io/gitimmersion-spanish/) · HTML
- [Git Magic](http://www-cs-students.stanford.edu/~blynn/gitmagic/intl/es/) — Ben Lynn · HTML

## Docker

<a id="docker"></a>

Contenedores, imágenes y flujos reproducibles para desarrollo y despliegue.

- [Docker en español](https://github.com/brunocascio/docker-espanol) — Bruno Cascio · HTML
- [Introducción a Docker](https://librosgratis.dev/books/docker-introduccion.pdf) — RedIRIS · PDF

## Linux y terminal

<a id="linux"></a>

Sistema, shell y fundamentos para moverte con soltura en entornos Unix.

- [El libro del administrador de Debian](https://debian-handbook.info/browse/es-ES/stable/) — Raphaël Hertzog y Roland Mas · HTML
- [El Manual de BASH Scripting Básico para Principiantes](https://es.wikibooks.org/wiki/El_Manual_de_BASH_Scripting_B%C3%A1sico_para_Principiantes) — Wikilibros · HTML

## SQL

<a id="sql"></a>

Consultas, modelado y fundamentos imprescindibles para cualquier stack.

- [Tutorial de SQL](http://www.desarrolloweb.com/manuales/9/) — Rubén Alvarez
- [Manual de SQL](http://jorgesanchez.net/manuales/sql/intro-sql-sql2016.html) — Jorge Sanchez Asenjo
- [Apuntes básicos de SQL](https://librosgratis.dev/books/sql-apuntes-basicos.pdf) — Unai Estébanez · PDF
- [Introducción al diseño de bases de datos](https://librosgratis.dev/books/bases-datos-diseno-introduccion.pdf) — Jordi Casas Roma · PDF

## NoSQL

<a id="nosql"></a>

MongoDB, Redis y modelos no relacionales para ampliar la caja de herramientas.

- [El pequeño libro de MongoDB](https://github.com/uokesita/the-little-mongodb-book) — Karl Seguin, traducido por Osledy Bazo · HTML
- [El pequeño libro de Redis en castellano](https://raulexposito.com/the-little-redis-book-en-castellano.html) — Karl Seguin, traducido por Raúl Expósito · HTML

## Sistemas operativos

<a id="sistemas-operativos"></a>

Procesos, memoria, archivos y concurrencia para entender qué hay debajo del stack.

- [Sistemas Operativos](https://librosgratis.dev/books/sistemas-operativos-wolf.pdf) — Gunnar Wolf, Esteban Ruiz, Federico Bergero, Erwin Meza · PDF

## Inteligencia Artificial

<a id="ia"></a>

Fundamentos de aprendizaje automático, agentes y razonamiento computacional.

- [Inteligencia Artificial: un enfoque moderno](https://iaarbook.github.io/) — Peter Norvig y Stuart Russell, adaptación abierta · HTML

## Metodologías de desarrollo

<a id="metodologias"></a>

Scrum, XP y prácticas de equipo para construir software de forma sostenible.

- [Guía Scrum](https://librosgratis.dev/books/guia-scrum-european.pdf) — EuropeanScrum.org · PDF
- [Scrum y XP desde las trincheras](https://librosgratis.dev/books/scrum-y-xp-desde-las-trincheras.pdf) — Henrik Kniberg · PDF

## Cómo proponer un recurso

Abre una issue o pull request con estos datos mínimos:

- Título
- Autor o proyecto
- Enlace oficial
- Formato disponible: PDF, HTML, ePub, etc.
- Confirmación de que es gratuito y está en español

Priorizamos recursos legales, gratuitos, estables y, cuando sea posible, hospedables como PDF sin superar los límites de assets de Cloudflare Workers.
